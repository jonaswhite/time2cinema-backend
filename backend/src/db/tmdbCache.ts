...
// 確保 TMDB 電影快取表存在
export async function ensureTMDBCacheTable() {
  try {
    // 檢查表是否存在
...
export async function migrateCacheToDatabase() {
  try {
    ...
    // 讀取所有 JSON 文件
    const files = fs.readdirSync(POSTER_CACHE_DIR).filter(file => file.endsWith('.json') && !file.includes('not_found_movies'));
    ...
    for (const file of files) {
      try {
        ...
        const movieData = await fs.readJSON(filePath) as TMDBMovie;
        ...
        `;
        const existingMovie = await pool.query(checkQuery, [movieData.title]);
...
// 從資料庫獲取電影信息
export async function getMovieFromCache(title: string): Promise<TMDBMovie | null> {
  try {
    const query = `
...
// 將電影信息保存到資料庫
export async function saveMovieToCache(movie: TMDBMovie): Promise<boolean> {
  try {
    // 檢查電影是否已存在
    const checkQuery = `
      SELECT id FROM tmdb_movie_cache WHERE title = $1
    `;
    const existingMovie = await pool.query(checkQuery, [movie.title]);
