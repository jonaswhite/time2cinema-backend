...
exports.movieTitleMapping = exports.movieTmdbMapping = void 0;
exports.getTmdbIdByTitle = getTmdbIdByTitle;
exports.getEnglishTitleByChinese = getEnglishTitleByChinese;
exports.saveTitleMapping = saveTitleMapping;
exports.ensureTitleMappingTable = ensureTitleMappingTable;
exports.syncTitleMappingToDatabase = syncTitleMappingToDatabase;
const db_1 = __importDefault(require("../db"));
exports.movieTmdbMapping = {
    // 票房榜前幾名的熱門電影
...
// 根據電影標題獲取 TMDB ID
function getTmdbIdByTitle(title) {
    // 嘗試直接匹配
    if (title in exports.movieTmdbMapping) {
        return exports.movieTmdbMapping[title];
    }
    // 嘗試忽略空格匹配
    const normalizedTitle = title.replace(/\s+/g, '');
    for (const [key, value] of Object.entries(exports.movieTmdbMapping)) {
...
async function getEnglishTitleByChinese(chineseTitle) {
    // 嘗試從內存映射表中直接匹配
    if (chineseTitle in exports.movieTitleMapping) {
        return exports.movieTitleMapping[chineseTitle];
    }
    // 嘗試忽略空格匹配
    const normalizedTitle = chineseTitle.replace(/\s+/g, '');
    for (const [key, value] of Object.entries(exports.movieTitleMapping)) {
        if (key.replace(/\s+/g, '') === normalizedTitle) {
            return value;
        }
    }
    ...
    `;
        const result = await db_1.default.query(query, [chineseTitle]);
...
// 將中英文電影標題映射保存到數據庫
async function saveTitleMapping(chineseTitle, englishTitle) {
    try {
        // 確保表存在
      ...
      RETURNING id
    `;
        const result = await db_1.default.query(query, [chineseTitle, englishTitle]);
        if (result.rows.length > 0) {
...
// 確保電影標題映射表存在
async function ensureTitleMappingTable() {
    try {
        const query = `
...
// 將內存映射表同步到數據庫
async function syncTitleMappingToDatabase() {
    try {
        // 確保表存在
        await ensureTitleMappingTable();
        // 將內存映射表中的所有映射保存到數據庫
        for (const [chineseTitle, englishTitle] of Object.entries(exports.movieTitleMapping)) {
