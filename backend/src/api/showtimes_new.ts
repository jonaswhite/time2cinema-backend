...
const formatShowtimesData = async (): Promise<TheaterShowtimes[]> => {
  try {
    ...
    // 先按電影院分組
    const showtimesByCinema: Record<string, Record<string, Showtime[]>> = {};
...
export const getShowtimesByTheater = async (req: Request, res: Response) => {
  try {
    ...
    const cinemaResult = await pool.query(
      'SELECT id, name FROM cinemas WHERE id = $1',
      [theaterId]
    );
    ...
    // 獲取該電影院的場次
    const showtimesResult = await pool.query(
      'SELECT date, time, movie_name FROM showtimes ' +
...
export const getShowtimesByMovie = async (req: Request, res: Response) => {
  try {
    ...
    let showtimesResult: any = { rowCount: 0, rows: [] };
