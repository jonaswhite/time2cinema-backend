...
exports.getShowtimesByMovie = exports.getShowtimesByDate = exports.formatDate = exports.getShowtimesByTheater = exports.getAllShowtimes = void 0;
...
const formatShowtimesData = async () => {
    try {
        ...
        const showtimesResult = await db_1.default.query('SELECT cinema_id, date, time, movie_name FROM showtimes WHERE DATE(date) >= DATE($1) ORDER BY cinema_id, date, time', [formattedDate]);
...
const getShowtimesByTheater = async (req, res) => {
    try {
        ...
        const showtimesResult = await db_1.default.query('SELECT cinema_id, date, time, movie_name FROM showtimes WHERE cinema_id = $1 AND DATE(date) >= DATE($2) ORDER BY date, time', [theaterId, formattedDate]);
        console.log(`找到 ${showtimesResult.rowCount} 筆場次資料`);
        // 獲取電影院信息
        ...
        // 按日期分組
        const showtimesByDate = {};
        showtimesResult.rows.forEach((showtime) => {
            const dateStr = showtime.date.toISOString().split('T')[0];
            if (!showtimesByDate[dateStr]) {
                showtimesByDate[dateStr] = [];
            }
