...
async function downloadBoxOfficeByTicketsDesc(outputDir = './') {
  ...
  try {
    ...
    // 直接使用 JavaScript 修改頁面，強制按票數降序排序
    console.log('嘗試直接修改頁面，強制按票數降序排序...');
    ...
    const clickResult = await page.evaluate(() => {
      ...
      for (let i = 0; i < headers.length; i++) {
        if (headers[i].textContent.includes('票數') && 
            !headers[i].textContent.includes('總票數') && 
            !headers[i].textContent.includes('票數變動')) {
          headers[i].click();
          return { method: 3, success: true, index: i };
        }
      }
    ...
    if (sortedData.data.length > 0) {
      ...
      if (firstTickets <= lastTickets && sortedData.data.length > 1) {
        console.log('警告: 排序可能不正確，嘗試在本地進行排序...');
