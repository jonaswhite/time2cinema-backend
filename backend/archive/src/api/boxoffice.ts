 ...
 */
async function scrapeBoxOfficeByTicketsDesc(maxPages = 20, waitTimeMs = 5000): Promise<BoxOfficeMovie[]> {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      if (ticketHeaders.length > 0) {
        (ticketHeaders[0] as HTMLElement).click();
      } else {
        const headers = Array.from(document.querySelectorAll('table th'));
        if (headers.length >= 9) {
          (headers[8] as HTMLElement).click();
        }
      }
    });
 ...
 * 獲取當前頁面的表格數據
 */
async function getPageData(page: Page): Promise<any[]> {
  try {
 ...
 */
function parseTableRows(rows: any[]): BoxOfficeMovie[] {
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const monday = getMonday(queryDate ? dayjs(queryDate) : dayjs());
    const dateStr = monday.format('YYYY-MM-DD');
    const cacheFile = path.join(CACHE_DIR, `boxoffice-${dateStr}.json`);
    ...
    // 寫入快取
    await fs.writeJSON(cacheFile, boxOfficeData, { spaces: 2 });
...
// 下載原始 JSON 數據路由
router.get('/download-json', async (req: Request, res: Response) => {
  try {
    const currentWeek = req.query.week ? parseInt(req.query.week as string) : 10;
