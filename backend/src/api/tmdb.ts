 ...
 * 使用多種方法嘗試獲取電影海報
 */
export async function smartSearchMoviePoster(movieTitle: string, releaseDate?: string): Promise<string | null> {
  ...
  try {
    ...
    try {
      const mappingResult = await pool.query(movieMappingQuery, [movieTitle]);
