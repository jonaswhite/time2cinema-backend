...
exports.ensureMoviesTable = ensureMoviesTable;
exports.migrateTMDBCacheToMovies = migrateTMDBCacheToMovies;
exports.runMoviesMigration = runMoviesMigration;
const pg_1 = require("pg");
const tmdbCache_1 = require("./tmdbCache");
// 線上資料庫配置
    ...
    `;
      ...
      `;
            ...
            const requiredColumns = [
                'english_title', 'poster_url', 'backdrop_url', 'overview', 'runtime', 'vote_average', 'genres', 'last_tmdb_check_at'
            ];
            for (const column of requiredColumns) {
                if (!columnNames.includes(column)) {
                    ...
                    if (column === 'vote_average')
                        dataType = 'NUMERIC(3,1)';
                    if (column === 'genres')
                        dataType = 'JSONB';
                    if (column === 'last_tmdb_check_at')
...
async function migrateTMDBCacheToMovies() {
    try {
        // 確保表存在
    ...
    `);
        ...
        `;
          ...
          `;
                    ...
                    const insertQuery = `
            INSERT INTO movies (
              tmdb_id, title, english_title, poster_url, backdrop_url, 
...
// 從 boxoffice 表遷移電影資料
async function migrateBoxofficeMovies() {
    try {
    ...
    `);
      ...
      `;
            ...
            if (checkResult.rows.length === 0) {
                ...
                const tmdbMovie = await (0, tmdbCache_1.getMovieFromCache)(movieName);
                if (tmdbMovie) {
                    // 插入到 movies 表
                    const insertQuery = `
            INSERT INTO movies (
...
// 從 showtimes 表遷移電影資料
async function migrateShowtimesMovies() {
    try {
...
// 執行遷移
async function runMoviesMigration() {
    try {
        console.log('開始電影資料遷移...');
        // 確保 TMDB 快取表存在
        await (0, tmdbCache_1.ensureTMDBCacheTable)();
        // 遷移電影資料
